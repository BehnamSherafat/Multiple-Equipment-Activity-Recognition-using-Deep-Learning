...
num_act_eqp = [4, 2]; % Number of Activities of Each Equipment (e.g., 4 activities for CAT259D, 3 activities for CAT938M, and 4 acts for Jackhammer)
...
layers = [
    layers(1:12)
    fullyConnectedLayer(num_act_eqp(2),'WeightLearnRateFactor',20,'BiasLearnRateFactor',20,'Name','fc1')
...
Y_Train1 = single(Y_Train(num_act_eqp(1)+1:num_act_eqp(1)+num_act_eqp(2),:));
...
Y_Val1 = single(Y_Val(num_act_eqp(1)+1:num_act_eqp(1)+num_act_eqp(2),:));
...
save('Trained_Net_2Eqp_2.mat','Trained_Net', '-v7.3');
