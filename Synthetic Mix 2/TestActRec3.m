...
load('Trained_Net_2Eqp_3.mat','Trained_Net');
...
subplot(2,1,1);
scatter(t,label_act{3},15,'filled','MarkerFaceColor',[1 0 0]);
title('Actual Labels for SkyJack Lift', 'FontSize', 20)
ax1 = gca;
set(ax1,'ytick',1:2)
set(ax1,'ylim',[1,2])
set(ax1,'yticklabel',{'Stop','Lifting Up'}, 'FontSize', 20)
xlim([0 T_test])
...
est_labels1 = [est_labels est_labels(end)*ones(1, size(label_act{3},2)-size(est_labels,2))];
...
filtered_sW1 = [filtered_sW filtered_sW(end)*ones(1, size(label_act{3},2)-size(filtered_sW,2))];
...
set(ax3,'ytick',1:2)
set(ax3,'ylim',[1,2])
set(ax3,'yticklabel',{'Stop','Lifting Up'},'FontSize',20)
xlim([0 T_test])
title('Predicted Labels for SkyJack Lift','FontSize',20)
...
% 
figure(2)
plotconfusion(categorical(label_act{3}),categorical(filtered_sW1))
